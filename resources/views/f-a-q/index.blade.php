				...
				<div class="panel-body">
					...
					<table class="data-table table table-striped table-bordered dt-responsive nowrap">
						<thead>
							<tr>
								<th>ID</th>
								<th width="300px"><a href="{{ $sortLinks['question']['url'] }}">Pregunta <i class="fa fa-{{ $sortLinks['question']['type'] }}"></i></a></th>					
								<th><a href="{{ $sortLinks['answer']['url'] }}">Respuesta <i class="fa fa-{{ $sortLinks['answer']['type'] }}"></i></a></th>					
						...
						<tbody>
							@foreach ($models as $model)
								<tr>
									...
									<td>{{{ str_limit(strip_tags($model->answer), 300) }}}</td>						
									<td>
										<div class="btn-group">
...
@section('footer')	
    ...
    <script>
	var ckeditorInit = function($el){
		$el.find('textarea').ckeditor({
			contentsCss : URL+'/css/app.css'
		});
	}
